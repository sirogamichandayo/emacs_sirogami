...
;; Enhance Heroku `run', `logs', and `pg:psql' commands with Emacs
;; equivalents.
...
(defun heroku-read-app (prompt-app)
  (if (or prompt-app (not (locate-dominating-file "." "Procfile")))
...
(defun heroku-run (&optional prompt-app)
  ...
  (interactive "P")
  (let* ((app (heroku-read-app prompt-app))
...
;;;###autoload
(defun heroku-logs (&optional prompt-app)
  "Open a buffer tailing logs for a given app."
  (interactive "P")
  (let* ((app (heroku-read-app prompt-app))
         (explicit-shell-file-name "heroku")
         (explicit-heroku-args (append (list "logs" "-t")
                                       (if app (list "-a" app)))))
    (shell (format "*heroku logs: %s*" app))))
